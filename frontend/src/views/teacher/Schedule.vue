    ...
    <el-dialog v-model="dialogVisible" title="课程详情" width="500px">
      <template v-if="currentCourse">
        <el-descriptions :column="1" border>
          ...
          <el-descriptions-item label="上课时间">{{ currentCourse.weekdayName }} {{
          ...
          <el-descriptions-item v-if="showTeacherInfoFlag && currentCourse.teacherName" label="授课教师">
...
const {userInfo, userId: storeUserId} = storeToRefs(userStore);

// Configuration for this TeacherScheduleView
const currentRole = 'teacher';
const pageTitle = '我的课表';
const showTeacherInfoFlag = false;
...
// Helper to map day value to name
...
const fetchSchedule = async () => {
  console.log("[fetchSchedule] Called. Checking conditions...");
  const currentUserIdInFetch = storeUserId.value;
  console.log("[fetchSchedule] semester.value:", semester.value, "Current userId in fetch:", currentUserIdInFetch);

  if (!semester.value) {
    ElMessage.warning('请先选择一个学期');
    console.log("[fetchSchedule] Returned: semester.value is falsy.");
    return;
  }

  if (!currentUserIdInFetch) {
    ElMessage.error('无法获取用户信息，无法加载课表。');
    scheduleList.value = [];
    loadingSchedule.value = false;
    console.log("[fetchSchedule] Returned: userId from storeUserId.value is falsy.");
    return;
  }
  ...
  try {
    ...
    console.log("[fetchSchedule] params for API:", params);
    let scheduleDataArray = [];

    console.log("[fetchSchedule] Proceeding to API call for role:", currentRole, "with userId:", currentUserIdInFetch);

    if (currentRole === 'student') {
      apiResponse = await getStudentSchedule(currentUserIdInFetch, params);
    } else if (currentRole === 'teacher') {
      apiResponse = await getTeacherSchedule(currentUserIdInFetch, params);
    } else {
...
// For week view: find course for a specific cell
...
// For week view: check if a cell is covered by a multi-slot course
const isCellCovered = (day, slot) => {
  return scheduleList.value.some(course =>
      course.weekday === day &&
      course.startSlot < slot && // Started before this slot
      course.endSlot >= slot    // But continues into or past this slot
  );
};

// For week view: calculate grid cell style for multi-slot courses
const getGridCellStyle = (day, slot) => {
  const course = findCourseForCell(day, slot);
  if (course && course.startSlot === slot) { // This is the starting cell of the course
    const duration = course.endSlot - course.startSlot + 1;
    return {
      gridRowStart: `span ${duration}`,
      backgroundColor: getCourseColor(course.courseId),
      zIndex: 5, // Bring multi-slot courses to the front
      alignSelf: 'stretch'
    };
  }
  return {};
...
const getCourseColor = (courseId) => {
  if (!courseColors[courseId]) {
    ...
    const colors = ['#E6F7FF', '#FFFBE6', '#F6FFED', '#FCF0FF', '#FFF0F0', '#E6FFF0']; // Pastel colors
...
const tableColumns = computed(() => [
  ...
  ...(showTeacherInfoFlag && currentRole === 'student' ? [{prop: 'teacherName', label: '教师', width: 100}] : []),
]);
...
watch(semester, (newSemester, oldSemester) => {
  if (newSemester && newSemester !== oldSemester) {
    const currentUserIdInSemesterWatch = storeUserId.value;
    if (currentUserIdInSemesterWatch) {
      console.log("[semester Watcher] Semester changed and userInfo exists (userId:", currentUserIdInSemesterWatch, "). Calling fetchSchedule.");
      fetchSchedule();
    } else {
      console.log("[semester Watcher] Semester changed, but waiting for userInfo (userId:", currentUserIdInSemesterWatch, ").");
    }
  }
});
...
onMounted(async () => {
  ...
  console.log("[onMounted] After fetchSemesters. Current semester:", semester?.value, "Current storeUserId at this point:", storeUserId?.value);

  const unwatchCombined = watch(
      [storeUserId, semester],
      ([newUserId, newSemesterVal], [oldUserId, oldSemesterVal]) => {
        console.log("[onMounted Combined Watcher] Watched values changed.", {
          newUserId,
          newSemesterVal,
          oldUserId,
          oldSemesterVal
        });
        if (newUserId && newSemesterVal && scheduleList.value.length === 0) {
          console.log("[onMounted Combined Watcher] Conditions met. Calling fetchSchedule using newUserId:", newUserId);
          fetchSchedule();
          unwatchCombined();
        } else {
          console.log("[onMounted Combined Watcher] Conditions not (yet) met.", {
            newUserId,
            newSemesterVal,
            scheduleLength: scheduleList.value.length
          });
        }
      },
      // { deep: true } // deep: true 对 ref<number> 和 ref<string> 可能不是必需的
  );
});
