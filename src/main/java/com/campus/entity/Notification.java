...
public class Notification implements Serializable {
    ...
    /**
     * 发布者名称 (非数据库字段，由 Service 层填充)
     */
    @TableField(exist = false)
    private String publisherName;

    /**
     * 发布时间 (非数据库字段，由 Service 层填充，通常使用 sendTime 或 createTime)
     */
    @TableField(exist = false)
    @JsonFormat(pattern = "yyyy-MM-dd HH:mm:ss", timezone = "GMT+8") // 保持格式一致
