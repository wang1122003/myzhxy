...
public class ActivityServiceImpl extends ServiceImpl<ActivityDao, Activity> implements ActivityService {
     ...
     */
    @Override
    public IPage<?> getActivityEnrollments(Long activityId, int page, int size) {
        // 获取活动信息
        Activity activity = getActivityById(activityId);
        if (activity == null) {
            log.error("Failed to get enrollments: Activity with ID {} not found", activityId);
            return new Page<>(page, size);
        }

        // 解析报名学生列表
        List<Map<String, Object>> participants = parseParticipantsJson(activity.getParticipantsJson(), activityId);
        ...
        if (start < total) {
            result.setRecords(participants.subList(start, end));
        } else {
