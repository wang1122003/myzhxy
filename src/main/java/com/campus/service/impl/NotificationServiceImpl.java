...
public class NotificationServiceImpl extends ServiceImpl<NotificationDao, Notification> implements NotificationService {
    ...
    public Notification getNotificationWithAttachments(Long id) {
        ...
        if (notification.getPublisherId() != null) {
            User publisher = userDao.selectById(notification.getPublisherId());
            if (publisher != null) {
                notification.setPublisherName(publisher.getUsername());
            } else {
                notification.setPublisherName("未知发布者");
            }
        }

        notification.setPublishTime(notification.getSendTime() != null ? notification.getSendTime() : notification.getCreateTime());
    ...
    public IPage<Notification> getNotificationPage(int pageNo, int pageSize, String type, String keyword, String status) {
        ...
        IPage<Notification> resultPage = page(page, queryWrapper);

        // 为列表中的每个通知填充非数据库字段
        if (resultPage != null && !CollectionUtils.isEmpty(resultPage.getRecords())) {
            // 批量获取所有相关的发布者信息以优化性能
            Set<Long> publisherIds = resultPage.getRecords().stream()
                    .map(Notification::getPublisherId)
                    .filter(Objects::nonNull)
                    .collect(Collectors.toSet());
            Map<Long, User> publisherMap = Collections.emptyMap();
            if (!publisherIds.isEmpty()) {
                List<User> publishers = userDao.selectBatchIds(publisherIds);
                publisherMap = publishers.stream().collect(Collectors.toMap(User::getId, u -> u));
            }

            Map<Long, User> finalPublisherMap = publisherMap; // Effectively final for lambda
            resultPage.getRecords().forEach(notification -> {
                // 填充发布时间 (使用 sendTime, 如果 sendTime 为空则使用 createTime)
                notification.setPublishTime(notification.getSendTime() != null ? notification.getSendTime() : notification.getCreateTime());

                // 填充发布者名称 
                if (notification.getPublisherId() != null) {
                    User publisher = finalPublisherMap.get(notification.getPublisherId());
                    if (publisher != null) {
                        notification.setPublisherName(publisher.getUsername());
                    } else {
                        notification.setPublisherName("未知发布者"); // Fallback
                    }
                } else {
                    notification.setPublisherName("系统"); // Or some default if publisherId is null
                }
            });
        }
