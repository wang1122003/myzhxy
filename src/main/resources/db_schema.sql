...
(
    ...
    `status`          VARCHAR(20) DEFAULT 'Active' COMMENT '在校状态: 在读, 已毕业, 休学等',
    `create_time`     DATETIME    DEFAULT CURRENT_TIMESTAMP,
    `update_time`     DATETIME    DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
...
(
    ...
    `status`        VARCHAR(20) DEFAULT 'Active' COMMENT '课程状态: 启用, 禁用',
    `create_time`   DATETIME DEFAULT CURRENT_TIMESTAMP,
    `update_time`   DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
...
(
    ...
    `status`         VARCHAR(20) DEFAULT 'Selected' COMMENT '状态：已选, 已退课',
    `create_time`    DATETIME DEFAULT CURRENT_TIMESTAMP,
    `update_time`    DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
...
(
    ...
    `status`       VARCHAR(20) DEFAULT 'Active' COMMENT '排课状态: 正常, 取消',
    `create_time`  DATETIME DEFAULT CURRENT_TIMESTAMP,
    `update_time`  DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    FOREIGN KEY (`course_id`) REFERENCES `course` (`id`) ON DELETE CASCADE,
...
(
    ...
    `status`               VARCHAR(20) DEFAULT 'Active' COMMENT '状态：进行中, 已取消, 已结束',
    `create_time`          DATETIME DEFAULT CURRENT_TIMESTAMP,
    `update_time`          DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
...
(
    ...
    `status`            VARCHAR(20) DEFAULT 'Confirmed' COMMENT '状态：已确认, 已取消, 待定',
    `create_time`       DATETIME DEFAULT CURRENT_TIMESTAMP,
    `update_time`       DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
...
(
    ...
    `status`           VARCHAR(20) DEFAULT 'Active' COMMENT '状态：正常, 已撤回',
...
(
    ...
    `status`        VARCHAR(20) DEFAULT 'Active' COMMENT '状态：正常, 已删除, 隐藏',
    `is_top`        TINYINT     DEFAULT 0 COMMENT '是否置顶：1-是，0-否',
...
(
    ...
    `like_count`  INT      DEFAULT 0 COMMENT '点赞数',
    `status`      VARCHAR(20) DEFAULT 'Active' COMMENT '状态：正常, 已删除',
